...
from wtforms import StringField, TextAreaField, SelectField, SubmitField, FloatField, IntegerField, BooleanField
...
class ClientEditForm(FlaskForm):
    ...
    name = StringField("Organization Name", validators=[DataRequired(), Length(max=128)])
    # Choices set in route
    industry = SelectField("Industry", validators=[DataRequired()])
    website = StringField("Website", validators=[Optional(), URL(), Length(max=128)])
    description = TextAreaField("Description", validators=[Optional()])
    public_profile_summary = TextAreaField("Public Profile Summary", validators=[Optional()])
    # Using TextArea for competitors, expecting JSON or comma-separated. Could enhance with JS later.
    competitors = TextAreaField("Competitors (JSON or comma-separated)", validators=[Optional()])
...
class LocationForm(FlaskForm):
    ...
    location_type = StringField("Location Type (e.g., HQ, Branch, Data Center)", validators=[Optional(), Length(max=32)])
...
class AssetForm(FlaskForm):
    ...
    asset_type = SelectField("Asset Type", choices=[('physical', 'Physical'), ('digital', 'Digital'), ('personnel', 'Personnel')], validators=[DataRequired()])
    description = TextAreaField("Description", validators=[Optional()])
    criticality_score = IntegerField("Criticality Score (1-10)", validators=[Optional(), NumberRange(min=1, max=10)])
    # Choices set in route
    location_id = SelectField("Location (Optional)", validators=[Optional()]) # Removed coerce=int
    # Using TextArea for technical details, expecting JSON.
    technical_details = TextAreaField("Technical Details (JSON format)", validators=[Optional()])
