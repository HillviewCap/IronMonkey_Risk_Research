...
from .forms import ClientOnboardingForm, ClientEditForm, LocationForm, ContactForm, AssetForm # Import new forms
from . import client_bp
from app.models.client import Client, ClientLocation, ClientContact, ClientAsset # Import models
...
def client_detail(client_id):
    ...
    contacts = client.contacts.order_by(ClientContact.last_name, ClientContact.first_name).all()
...
def edit_client(client_id):
    ...
    try:
        ...
        current_app.logger.error(f"Failed to load industries for edit form: {e}", exc_info=True)
        form.industry.choices = [("", "Error loading industries")]
        ...
        try:
            ...
            if competitors_raw:
                try:
                    ...
                    client.competitors = [c.strip() for c in competitors_raw.split(',') if c.strip()]
            else:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error updating client {client_id}: {e}", exc_info=True)
    ...
    return render_template('client/edit_client.html', title=f"Edit Client: {client.name}", form=form, client=client)
...
def add_location(client_id):
    ...
    # Load countries from config
    form.country.choices = current_app.config.get('COUNTRIES', [("", "Error loading countries")])
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error adding location for client {client_id}: {e}", exc_info=True)
            flash('Error adding location. Please check logs.', 'danger')
            # Re-render form on exception during POST
            return render_template('client/location_form.html', title='Add New Location', form=form, client=client)

    # Handle GET request or failed validation
    return render_template('client/location_form.html', title='Add New Location', form=form, client=client)


@client_bp.route('/clients/<int:client_id>/locations/<int:location_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_location(client_id, location_id):
    """Handle editing an existing location for a client."""
    location = ClientLocation.query.filter_by(id=location_id, client_id=client_id).first_or_404()
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error updating location {location_id} for client {client_id}: {e}", exc_info=True)
            flash('Error updating location. Please check logs.', 'danger')



@client_bp.route('/clients/<int:client_id>/locations/<int:location_id>/delete', methods=['POST'])
@login_required
def delete_location(client_id, location_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error deleting location {location_id} for client {client_id}: {e}", exc_info=True)
...
def add_contact(client_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error adding contact for client {client_id}: {e}", exc_info=True)
            flash('Error adding contact. Please check logs.', 'danger')
            # Re-render form on exception during POST
            return render_template('client/contact_form.html', title='Add New Contact', form=form, client=client)

    # Handle GET request or failed validation
    return render_template('client/contact_form.html', title='Add New Contact', form=form, client=client)


@client_bp.route('/clients/<int:client_id>/contacts/<int:contact_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_contact(client_id, contact_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error updating contact {contact_id} for client {client_id}: {e}", exc_info=True)
            flash('Error updating contact. Please check logs.', 'danger')

    return render_template('client/contact_form.html', title='Edit Contact', form=form, client=client, contact=contact)



@client_bp.route('/clients/<int:client_id>/contacts/<int:contact_id>/delete', methods=['POST'])
@login_required
def delete_contact(client_id, contact_id):
    ...
    try:
        ...
        db.session.commit()
        flash(f'Contact "{contact.first_name} {contact.last_name}" deleted successfully!', 'success')
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error deleting contact {contact_id} for client {client_id}: {e}", exc_info=True)
...
def add_asset(client_id):
    ...
    form.location_id.choices = [('', '-- Select Location (Optional) --')] + [
        (loc.id, loc.name) for loc in client.locations.order_by(ClientLocation.name).all()
    ]
    ...
    if form.validate_on_submit():
        try:
            ...
            # Handle technical details JSON
            tech_details_raw = form.technical_details.data
            if tech_details_raw:
                try:
                    new_asset.technical_details = json.loads(tech_details_raw)
                except json.JSONDecodeError:
                    flash('Technical details must be valid JSON.', 'warning')
                    # Re-render form with error, don't proceed with saving
                    return render_template('client/asset_form.html', title='Add New Asset', form=form, client=client)
            else:
                new_asset.technical_details = None
...
@client_bp.route('/clients/<int:client_id>/assets/<int:asset_id>/edit', methods=['GET', 'POST'])
@login_required
def edit_asset(client_id, asset_id):
    ...
    # Convert JSON technical_details back to string for TextArea
    if asset.technical_details:
        initial_tech_details = json.dumps(asset.technical_details, indent=2)
    else:
        initial_tech_details = ''

    form = AssetForm(obj=asset, technical_details=initial_tech_details) # Pre-populate
    ...
    if form.validate_on_submit():
        try:
            ...
            if tech_details_raw:
                try:
                    asset.technical_details = json.loads(tech_details_raw)
                except json.JSONDecodeError:
                    flash('Technical details must be valid JSON.', 'warning')
                    # Re-render form with error
                    return render_template('client/asset_form.html', title='Edit Asset', form=form, client=client, asset=asset)
            else:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error updating asset {asset_id} for client {client_id}: {e}", exc_info=True)
...
@client_bp.route('/clients/<int:client_id>/assets/<int:asset_id>/delete', methods=['POST'])
@login_required
def delete_asset(client_id, asset_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error deleting asset {asset_id} for client {client_id}: {e}", exc_info=True)
    ...
    return render_template('client/location_form.html', title='Edit Location', form=form, client=client, location=location)
