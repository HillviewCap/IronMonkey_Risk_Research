...
class Config:
    ...
    encoded_postgres_password = (
        urllib.parse.quote_plus(clean_postgres_password)
        if clean_postgres_password
        else ""
    )
    ...
    encoded_mongo_password = (
        urllib.parse.quote_plus(mongo_password) if mongo_password else ""
    )
    ...
    encoded_redis_password = (
        urllib.parse.quote_plus(redis_password) if redis_password else ""
    )
    REDIS_URL = os.environ.get(
        "REDIS_URL",
        (
            f"redis://:{encoded_redis_password}@{os.environ.get('REDIS_HOST', 'localhost')}:{os.environ.get('REDIS_PORT', '6379')}/0"
            if encoded_redis_password
            else f"redis://{os.environ.get('REDIS_HOST', 'localhost')}:{os.environ.get('REDIS_PORT', '6379')}/0"
        ),
    )
    ...
    # Configuration settings continue here
...
class TestingConfig(Config):
    ...
    print(
        f"TestingConfig DEBUG: REDIS_PASSWORD available: {'Yes' if _redis_password else 'No'}"
    )
    ...
    if _base_redis_url_env:
        try:
            ...
            print(
                f"TestingConfig Warning: Failed to parse REDIS_URL '{_base_redis_url_env}'. Falling back. Error: {e}"
            )
            # Fallback logic
            _redis_host = os.environ.get("REDIS_HOST", "localhost")
            _redis_port = os.environ.get("REDIS_PORT", "6379")
            _encoded_redis_password = (
                urllib.parse.quote_plus(_redis_password) if _redis_password else ""
            )
            REDIS_URL = (
                f"redis://:{_encoded_redis_password}@{_redis_host}:{_redis_port}/1"
                if _encoded_redis_password
                else f"redis://{_redis_host}:{_redis_port}/1"
            )
            print(
                f"TestingConfig: Constructed REDIS_URL from fallback components: {REDIS_URL}"
            )
    else:
        ...
        print(
            f"TestingConfig: Constructed REDIS_URL from components (REDIS_URL env var not found): {REDIS_URL}"
        )
    # Clean up temporary variables used for class-level construction
    del _base_redis_url_env
    if "_parsed_redis" in locals():
        del _parsed_redis
    if "_redis_host" in locals():
        del _redis_host
    if "_redis_port" in locals():
        del _redis_port
    if "_redis_password" in locals():
        del _redis_password
    if "_encoded_redis_password" in locals():
        del _encoded_redis_password
    ...
    try:
        ...
        print(
            "TestingConfig Warning: redis package not installed. Cannot configure SESSION_REDIS."
        )
        SESSION_REDIS = None
    except Exception as e:
        print(
            f"TestingConfig Warning: Failed to create SESSION_REDIS from URL '{REDIS_URL}'. Error: {e}"
        )
        SESSION_REDIS = None
    # --- End Redis Test DB ---
