...
def test_login_page_loads(client):
    ...
    assert b"Sign In" in response.data # Check for expected text in the rendered template
...
def test_login_success(client, test_user):
    ...
    }, follow_redirects=True) # Follow redirects to check final destination

    assert response.status_code == 200 # Should land on dashboard
    # Check if it landed on the dashboard page
    assert b"Dashboard" in response.data # Assuming dashboard template has "Dashboard" title/text
...
def test_register_success(client, db_session): # Use db_session to check DB state
    """
    ...
    assert b"Congratulations, you are now a registered user!" in response.data # Check flash
...
def test_session_debug_authenticated(client, test_user, login):
    ...
    assert data['session']['_user_id'] == str(test_user.id) # Flask-Login stores ID as string
...
def test_register_fail_password_complexity(client):
    ...
    assert response_dashboard.location.startswith(url_for('auth.login', _external=False))